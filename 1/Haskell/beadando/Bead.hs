...
-- parseSafe _ [] = Right []
-- parseSafe oprt s = parseSafe' oprt (words s)

-- parseSafe' :: Read a => OperatorTable a -> [String] -> ShuntingYardResult [Tok a]
-- parseSafe' _ [] = Right []
-- parseSafe' oprt (x:xs)
--   | x == "(" = csomagol [BrckOpen] (parseSafe' oprt xs)
--   | x == ")" = csomagol [BrckClose] (parseSafe' oprt xs)
--   | length x == 1 && isJust (operatorFromChar oprt (head x)) = csomagol [fromJust (operatorFromChar oprt (head x))] (parseSafe' oprt xs)
--   | isJust val = csomagol [TokLit (fromJust val)] (parseSafe' oprt xs)
--   | otherwise = Left ParseError
--   where 
--     val = readMaybe x

-- csomagol :: [a] -> ShuntingYardResult [a] -> ShuntingYardResult [a]
-- csomagol _ (Left err) = Left err
-- csomagol first (Right second) = Right (first ++ second)


-- shuntingYardSafe :: [Tok a] -> ShuntingYardResult ([a], [Tok a])
-- shuntingYardSafe [] = Right ([], [])
-- shuntingYardSafe tok = helper tok [] [] where
--   helper [] litList opList
--     | null opList = Right (litList, opList)
--     | otherwise = Left NoClosingParen
--   helper ((TokLit x):xs) litList opList = helper xs (x : litList) opList
--   helper (BrckOpen:xs) litList opList = helper xs litList (BrckOpen : opList)
--   helper (BrckClose:xs) litList opList = kiertekeles xs litList opList where
--       kiertekeles _ _ [] = Left NoOpeningParen
--       kiertekeles xs litList (BrckOpen:ys) = helper xs litList ys
--       kiertekeles xs (y1:y2:ys) (TokBinOp f _ _ _:zs) = kiertekeles xs (f y2 y1 : ys) zs
--       kiertekeles _ _ _ = Left OperatorOrClosingParenExpected
--   helper (TokBinOp _ _ _ _ : xs) [] _ = Left LiteralOrOpeningParenExpected
--   helper (TokBinOp f szimb kot dir : xs) litList opList = helper xs newLitList (TokBinOp f szimb kot dir : newOpList) where
--     (newLitList, newOpList) = kiertekeles litList opList kot dir where
--       kiertekeles litList [] _ _ = (litList, [])
--       kiertekeles litList (TokBinOp f2 szimb2 kot2 dir2 : ys) kot dir
--         | kot > kot2 || (kot == kot2 && dir == InfixR) = (litList, TokBinOp f2 szimb2 kot2 dir2 : ys)
--         | otherwise = kiertekeles (applyOperator f2 litList) ys kot dir where
--             applyOperator op (x:y:zs) = (op y x) : zs
--             applyOperator _ _ = error "Not enough operands"
--       kiertekeles litList opList _ _ = (litList, opList)
