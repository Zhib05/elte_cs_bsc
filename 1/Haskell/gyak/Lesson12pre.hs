...
minimumBy' c (x:xs) = helper c x xs where
    helper _ m [] = m
    helper c m (x:xs)
        | c m x == GT = helper c x xs
        | otherwise = helper c m xs
...
sortBy' c ls = helper c ls ((length ls) - 1) where
    helper _ ls 0 = ls
    helper c ls i = helper c (helper2 c ls) (i - 1) where
        helper2 c [] = []
        helper2 c [x] = [x]
        helper2 c (x:y:xs)
