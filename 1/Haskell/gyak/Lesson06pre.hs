...
mergeSort [] = []
mergeSort [x] = [x]
mergeSort list = merge (mergeSort left) (mergeSort right) where
...
combinations n xs
    | n < 0 = []
    | n == 0 = []
    |otherwise = comb n xs where
        comb 0 _ = [[]]
        comb k [] = []
        comb k (y:ys) = [ y : zs | zs <- comb (k-1) ys ] ++ comb k ys
...
deletions :: [a] -> [[a]]
deletions [] = []
deletions xs = [ take i xs ++ drop (i+1) xs | i <- [0..length xs - 1] ] 
...
type IntToupleList = [(Int,Int)]

person2 :: (Name, Address, Phone, Email, Age)
person2 = ("Én","Alma utca 10","06-20-nemmondommeg","alma@alma.hu",20)
