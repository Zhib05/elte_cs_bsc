...
void CMyApp::InitTextures()
{
	...
	ImageRGBA tableImage = ImageFromFile("Assets/green_fabric.jpg");
	glCreateTextures(GL_TEXTURE_2D, 1, &m_tableTextureID);
	glTextureStorage2D(m_tableTextureID, NumberOfMIPLevels(tableImage), GL_RGBA8, tableImage.width, tableImage.height);
	glTextureSubImage2D(m_tableTextureID, 0, 0, 0, tableImage.width, tableImage.height, GL_RGBA, GL_UNSIGNED_BYTE, tableImage.data());
	glGenerateTextureMipmap(m_tableTextureID);

	ImageRGBA cardImage = ImageFromFile("Assets/card_back.png");
	...
	ImageRGBA handImage = ImageFromFile("Assets/hand.png");
	glCreateTextures(GL_TEXTURE_2D, 1, &m_handTextureID);
	glTextureStorage2D(m_handTextureID, NumberOfMIPLevels(handImage), GL_RGBA8, handImage.width, handImage.height);
	glTextureSubImage2D(m_handTextureID, 0, 0, 0, handImage.width, handImage.height, GL_RGBA, GL_UNSIGNED_BYTE, handImage.data());
	glGenerateTextureMipmap(m_handTextureID);

	ImageRGBA chipImage = ImageFromFile("Assets/pokerchip.jpg");
	glCreateTextures(GL_TEXTURE_2D, 1, &m_chipTextureID);
	glTextureStorage2D(m_chipTextureID, NumberOfMIPLevels(chipImage), GL_RGBA8, chipImage.width, chipImage.height);
	glTextureSubImage2D(m_chipTextureID, 0, 0, 0, chipImage.width, chipImage.height, GL_RGBA, GL_UNSIGNED_BYTE, chipImage.data());
...
bool CMyApp::Init()
{
	...
	m_cards.clear();
	for (float i : {-4.0f, -2.0f, 0.0f, 2.0f, 4.0f}) {
		CardState card;
		card.position = glm::vec3(i, 0.0f, 0.0f);
		card.type = 0;
...
void CMyApp::RenderTable() {
	...
	glm::mat4 World = glm::translate(glm::vec3(0.0f, -0.1f, 0.0f)) * glm::scale(glm::vec3(30.0f, 1.0f, 21.0f));
	DrawObject(m_quadGPU, World);
}

void CMyApp::RenderCard() {
	glBindTextureUnit(0, m_cardTextureID);
	for (const CardState& card : m_cards) {
		glm::mat4 World = glm::translate(glm::vec3(card.position)) * glm::scale(glm::vec3(1.6f, 1.0f, 2.48f));
		DrawObject(m_quadGPU, World);
	}
}
...
void CMyApp::RenderHand() {
	...
	glm::mat4 World = glm::translate(glm::vec3(-3.0f, 2.0f, 12.0f)) * glm::rotate(m_handRotation, glm::vec3(0.0f, 1.0f, 0.0f)) * glm::scale(glm::vec3(3.0f));
...
void CMyApp::RenderChip() {
	...
	float radius = 4.0f;
	float time = m_ElapsedTimeInSec;

	for (int j = 0; j < chipK; j++) {
		float orbitAngle = (j / (float)chipK) * 2.0f * glm::pi<float>() + time;
...
void CMyApp::RenderGUI()
{
	...
	ImGui::SliderInt("Torony zsetonok (N)", &chipN, 1, 20);
	ImGui::SliderInt("Keringo zsetonok (K)", &chipK, 1, 20);
