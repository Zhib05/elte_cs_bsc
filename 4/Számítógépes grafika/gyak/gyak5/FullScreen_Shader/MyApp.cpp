...
void CMyApp::Render()
{
	...
	glUniform1f(glGetUniformLocation(m_programID, "m_translate_X"), m_translate_X);
	glUniform1f(glGetUniformLocation(m_programID, "m_translate_Y"), m_translate_Y);
	...
	{
		glUniform1f(glGetUniformLocation(m_programID, "m_translate_X"), m_quadPositions[i].x);
		glUniform1f(glGetUniformLocation(m_programID, "m_translate_Y"), m_quadPositions[i].y);
...
void CMyApp::RenderGUI()
{
	...
	{
		ImGui::SliderFloat("Translate X", &m_translate_X, -1.0f, 1.0f);
		ImGui::SliderFloat("Translate Y", &m_translate_Y, -1.0f, 1.0f);
	}
	ImGui::End();
}
...
{	
	if (!key.repeat)
	{
		...
		{
			// CTRL + F5 lenyomására újratöltjük a shadereket 
			CleanShaders();
			InitShaders();
		}
	}
...
{
	// Ha a bal egérgombot nyomtuk le
	if (mouse.button == SDL_BUTTON_LEFT)
	{
		if (m_quadCount < 100)
		{
			// Pixel koordináták átszámítása OpenGL NDC koordinátákra (-1 és 1 közé)
			float ndcX = (mouse.x / (float)m_windowWidth) * 2.0f - 1.0f;
			float ndcY = 1.0f - (mouse.y / (float)m_windowHeight) * 2.0f;

			// Pozíció mentése a meglévő glm::vec2 típussal
			m_quadPositions[m_quadCount] = glm::vec2(ndcX, ndcY);
			m_quadCount++;
		}
	}
}
