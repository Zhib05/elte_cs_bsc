...
int main( int argc, char* args[] )
{
	...
	win = SDL_CreateWindow( "Hello SDL&OpenGL!",		// az ablak fejléce 
							...
							720,						// és magassága 
							SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE);			// megjelenítési tulajdonságok 
	...
	SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "Renderer: %s", renderer);
	SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION, "GLSL version: %s", glsl_version);
	...
	// Az Imgui-n megjelenő szöveg kinézetének és betűméretének a beállítása 
	const std::string fontPath = "C:\\Windows\\Fonts\\arial.ttf"; // Útvonal a használandó ttf fájlhoz 
	
	// Ezt az értéket kell megváltoztatni ha a betűméretet akarjuk változtatni 
	const float fontSize = 14.0f * SDL_GetWindowDisplayScale(win);
	
	if (std::filesystem::exists(fontPath)) {
		// Ha a megadott fontPath létezik, állítsuk be azt és a méretét is 
		ImGui::GetIO().Fonts->AddFontFromFileTTF(fontPath.c_str(), fontSize);
	}
	else {
		// Ha nem létezik, csak a szöveg méretet állítjuk be 
		ImFontConfig config;
...

	{
		...
		while (!quit)
		{
			...
			while ( SDL_PollEvent(&ev) )
			{
				...
				switch (ev.type)
				{
					...
					case SDL_EVENT_KEY_DOWN:
						...
						if ( ( ev.key.key == SDLK_RETURN )  // Enter le lett nyomva, ... 
							 && ( ev.key.mod & SDL_KMOD_ALT )   // az ALTal együtt, ... 
							 && !( ev.key.mod & ( SDL_KMOD_SHIFT | SDL_KMOD_CTRL | SDL_KMOD_GUI ) ) ) // de más modifier gomb nem lett lenyomva. 
						{
							Uint32 FullScreenSwitchFlag = ( SDL_GetWindowFlags( win ) & SDL_WINDOW_FULLSCREEN ) ? 0 : SDL_WINDOW_FULLSCREEN;
						...
						if ( ( ev.key.key == SDLK_F1 )  // F1 le lett nyomva, ... 
							 && ( ev.key.mod & SDL_KMOD_CTRL )   // az CTRLal együtt, ... 
							 && !( ev.key.mod & ( SDL_KMOD_SHIFT | SDL_KMOD_ALT | SDL_KMOD_GUI ) ) ) // de más modifier gomb nem lett lenyomva. 
						{
							ShowImGui = !ShowImGui;
			...
			static Uint64 LastTick = SDL_GetTicks(); // statikusan tároljuk, mi volt az előző "tick". 
			Uint64 CurrentTick = SDL_GetTicks(); // Mi az aktuális. 
			...
			ImGui_ImplSDL3_NewFrame(); // Ezután lehet imgui parancsokat hívni, egészen az ImGui::Render()-ig 
